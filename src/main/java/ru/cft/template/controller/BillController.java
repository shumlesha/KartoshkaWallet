...
public class BillController {
     ...
     * @param createBillModel модель создания счета на оплату
    ...
    public ResponseEntity<DefaultResponse<BillDTO>> createBill(@AuthenticationPrincipal SessionUser sessionUser,
                                                               @Validated @RequestBody CreateBillModel createBillModel) {
        Bill createdBill = billService.createBill(sessionUser, createBillModel);

        log.info("Created bill with id: {}", createdBill.getId());
    ...
    public ResponseEntity<DefaultResponse<Page<BillDTO>>> getBills(@AuthenticationPrincipal SessionUser sessionUser,
        ...
        return ResponseEntity.ok(DefaultResponseBuilder.success(
                ...
                billService.getBills(sessionUser, billFilter, pageable).map(billMapper::toDTO)
        ));
    }
